odoo.define('flexibite_ee_advance.UnlockPosScreen', function(require) {
    'use strict';
        ...
        async toggle(key) {
            ...
            if (confirmed){
                ...
                if (this.env.pos.config.module_pos_hr){
                    if(user && !user.pin){
                        alert("Please Set Security Pin for Perticular Employee !");
                        return
                    }
                }else{
                    if(user && this.env.pos.user.pin){
                        ...
                        alert("Please Set Security Pin for Perticular User !");
                        return
                    }
                }
                var incript_password = Sha1.hash(payload)
                if(user.pin == incript_password){
                    var params = {
                        model: 'pos.session',
                        method: 'write',
