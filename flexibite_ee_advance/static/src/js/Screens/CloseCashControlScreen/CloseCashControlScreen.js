odoo.define('flexibite_ee_advance.CloseCashControlScreen', function (require) {
    'use strict';
    ...
    class CloseCashControlScreen extends IndependentToOrderScreen {
        ...
        onClickCloseSession(){
            ...
            _.each(self.state.closeInputLines, function(line){
               if(line.line_total > 0){
                   dict.push({
                       'coin_value':self.env.pos.db.thousandsDecimalChanger(line.coin_value),
                       'number_of_coins': self.env.pos.db.thousandsDecimalChanger(line.number_of_coins),
        ...
        async closePosSession(){
            ...
            return await this.rpc(params, {async: false}).then(function(res){});
        }
