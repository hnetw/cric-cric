    ...
    class giftCardCreateScreen extends PosComponent {
        ...
        async confirm() {
            if(this.state.SelectCustomer == ''){
                ...
                this.props.resolve({ confirmed: true, payload:{card_no: this.state.CardNumber, customer_id: this.state.SelectCustomer, expire_date: this.state.ExpireDate, card_value: this.state.Amount, card_type: this.state.SelectCardType,} });
                this.trigger('close-temp-screen');
            }
        }
