odoo.define('flexibite_ee_advance.CustomOrderScreen', function(require) {
    'use strict';
    ...
    class CustomOrderScreen extends ControlButtonsMixin(PosComponent) {
        ...
        constructor() {
            ...
            this.state = useState({
                            ...
                            categoryMode: false,
                            categories:false,
                            replaceBOMMode:false,
                            selectedBomCategoryId:false,
                        });
        ...
        _switchCategory(event) {
            this.state.selectedBomCategoryId = event.detail;
            this.render();
        }
        _addBomLine(product){
        ...
        _addCustomisedMaterialline(){
            var materiallines = this.props.orderline.materiallines;
            if(materiallines.length != 0){
                for(var i = 0; i < materiallines.length; i++){
                    ...
                    this.currentOrder.add_material(materialline.product,{
                         ...
                         replaceable_category_ids: materialline.replaceable_category_ids,
                         replaceable_by: materialline.replaceable_by,
                         bom_base_price: materialline.bom_base_price,
                    });
                }
            }
            this._changeButtonEnable();
        ...
        _onClickAdd(){
            var price = this.props.orderline.get_lst_price();
            var price_extra = this.props.orderline.price_extra;
            var materiallines = this.currentOrder.get_materiallines();
            this.props.orderline.set_materiallines([]);
            this.props.orderline.set_materiallines(materiallines);
            this.props.orderline.price_manually_set = true;
            this.props.orderline.set_unit_price(this.currentOrder.m_get_total_without_tax() + price + price_extra)
        ...
        get bomCategories(){
            const { replaceable_category_ids } = this.currentLine;
            let categList = [];
            for (let categ of replaceable_category_ids){
                categList.push(this.env.pos.db.get_category_by_id(categ));
            }
        }
        async _clickReplaceProduct(event){
            ...
            if(this.currentLine && this.currentLine.replaceable_by == "category") {
                const { replaceable_category_ids } = this.currentLine;
                let categList = [];
                for (let categ of replaceable_category_ids){
                    categList.push(this.env.pos.db.get_category_by_id(categ));
                }
                this.state.categories = categList;
                this.state.selectedBomCategoryId= categList[0].id;
                this.state.categoryMode = true;
                this.state.replaceMode = true;
                this.state.replaceBOMMode = 'category';
            }else{
                this.state.replaceBOMMode = 'product';
            }
            if(this.state.replaceMode){
                this.state.replaceMode = false;
                this.state.replaceMode = true;
                this.state.categoryMode = false;
            }
            this._changeButtonEnable();
        }
        async _clickReplaceProductItem(event){
            ...
            if (this.env.pos.config.product_configurator && _.some(product.attribute_line_ids, (id) => id in this.env.pos.attributes_by_ptal_id)) {
                ...
                if (confirmedPopup){
                    ...
                    var line = this.currentOrder.get_selected_materialline();
                    line.set_replaced_product_id(product.id);
                    if(product.lst_price > line.bom_base_price){
                        price_extra = product.lst_price - line.bom_base_price;
                    }
                    if(line.quantity == 0){
                        line.set_quantity(1);
                    }
    ...
    CustomOrderScreen.template = 'CustomOrderScreen';

    Registries.Component.add(CustomOrderScreen);
