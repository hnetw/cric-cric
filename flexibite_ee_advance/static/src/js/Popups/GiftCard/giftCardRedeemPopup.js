odoo.define('flexibite_ee_advance.giftCardRedeemPopup', function(require) {
    'use strict';
    ...
    class giftCardRedeemPopup extends AbstractAwaitablePopup {
        constructor() {
            super(...arguments);
            this.state = useState({ GiftCardNumber: ''});
        ...
        async confirm(){
            ...
            if (gift_card && gift_card[0]){
                if (this.state.GiftCardAmount > gift_card[0].card_value){
                    ...
                    $('#lbl_card_no').html('');
                    $('#lbl_set_customer').html('Please enter amount below card value.');
                    $('#in_balance').html('');
                }
                else if (this.state.GiftCardAmount > self.env.pos.get_order().get_due()){
                    $('#text_redeem_amount').focus();
                    $('#lbl_card_no').html('');
                    $('#lbl_set_customer').html('Please enter valid amount.');
                    $('#in_balance').html('');
                }else{
                ...
                if(res.length > 0){
                    if (res[0]){
                        ...
                        if (self.env.pos.get_order().get_due() < res[0].card_value){
                            self.state.GiftCardAmount = self.env.pos.format_currency_no_symbol(self.env.pos.get_order().get_due())
                        }
                        if (res[0].card_value < self.env.pos.get_order().get_due()){
                            self.state.GiftCardAmount = self.env.pos.format_currency_no_symbol(res[0].card_value)
                        }
                    ...
                    $('#lbl_card_no').html('');
                    $('#lbl_set_customer').html('Barcode not found or gift card has been expired.');
                    $('#in_balance').html('');
