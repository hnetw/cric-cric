...
class PosSession(models.Model):
    ...
    increment_number = fields.Integer(string="Increment Number", default=0,
    ...
    def take_money_in_out(self, payload):
        for each in self:
            try:
                ...
                res = self.env['cash.box.out'].create({'name': payload.get('reason'), 'amount': payload.get('amount')})
    ...
    def send_email_z_report(self, id):
        ...
        try:
            ...
            report_id = self.env['ir.actions.report'].search(
                [('report_name', '=', 'flexibite_ee_advance.pos_z_report_template')])
    ...
    def get_net_gross_total(self):
        return self.get_gross_total() - self.get_total_tax()
    ...
    def get_total_tax(self):
        total_tax = 0.0
        total_tax += sum([order.amount_tax for order in self.env['pos.order'].search([('session_id', '=', self.id)])])
        return total_tax
    ...
    def get_vat_tax(self):
        ...
        for tax in self.env['account.tax'].browse(tax_list):
            total_tax = 0.00
            net_total = 0.00
            pos_line_ids = self.env['pos.order.line'].search(
                [('order_id', 'in', [order.id for order in self.order_ids])])
            for line in pos_line_ids.filtered(lambda line: tax in line.tax_ids_after_fiscal_position):
